...
│   ├── manifest.webmanifest    朝ニュースを単体アプリとして入れるための定義
│   ├── gen-icons.cjs           アイコン生成（node news/gen-icons.cjs）
│   ├── icon*.png / icon.svg    横帯＝見出しが並ぶ形。縦帯の夏休みダイヤと区別する
│   ├── index.html             （自動生成）最新号
...
**朝ニュースと夏休みダイヤは別々のアプリとして入る。** それぞれのURLを開いて
ホーム画面に追加する。`start_url` が違うので2つ並んで共存する。

| アプリ | 開くURL | アイコン |
|---|---|---|
| 朝ニュース | `…/news/` | 横帯（見出しが5行） |
| 夏休みダイヤ | `…/`（ルート） | 縦帯（時刻表） |

- **Android / Chrome**：メニュー →「アプリをインストール」
- **iPhone / Safari**：共有 →「ホーム画面に追加」

Service Workerはルートに1つだけ置き、両方をキャッシュする。
`/news/` の**ページだけ**ネットワーク優先（毎朝変わるため）、
アイコンとmanifestとアプリシェルはキャッシュ優先。
